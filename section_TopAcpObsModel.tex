...
\label{sec:AcpModelObs}
	...
	\label{ssec:AcpObs}
		...
		The probed observables are roughly classified to two type. The first one is the CP-odd observables which is T-odd and the other is CP-odd observable which is T-even. The even/odd means the $\hat{CP}$/$\hat{T}$ operator's eigenstate with eigenvalue $+1$/$-1$. The triple product type observable(for example, the $O_{3,6,12,14}$ we use in the 2016 analysis) is T-odd observable. The introduction of T-odd CP-odd observable is mentioned in section***** before, so I will put emphasis on T-even observables, and make a simple test to T-odd observables in following contents.
		...
		\label{sssec:AcpObs_class_test}
			...
			\begin{figure}[H]
			\centering
				\subfigure[semileptonic decay $t\bar{t}$, SM]{\includegraphics[width=0.32\textwidth]{Figures/Observables/lepEtAsym_10_7-semi_SM_0j_NoSel_comb.pdf}}
			    \subfigure[semileptonic decay $t\bar{t}$, CEDM]{\includegraphics[width=0.32\textwidth]{Figures/Observables/lepEtAsym_10_7-semi_CEDM5_0j_NoSel_comb.pdf}}
			    \subfigure[semileptonic decay $t\bar{t}$, 2HDM]{\includegraphics[width=0.32\textwidth]{Figures/Observables/lepEtAsym_10_7-semi_2HDM_NLO_m1_0j_NoSel_comb.pdf}}\\
			...
			For $A_1$, it is simple to measure event by event and calculate their expectation value. However, for $A_2$, it is more complex because of measurement on the $t\bar{t}$ semileptonic decay.
