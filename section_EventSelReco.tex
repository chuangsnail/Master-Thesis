...
\begin{figure}[H]
\centering
	\subfigure[muon channel]{\includegraphics[width=0.45\textwidth]{Figures/EventSelReco/chi2_NC_long_HadTop_mu.pdf}}
    \subfigure[electron channel]{\includegraphics[width=0.45\textwidth]{Figures/EventSelReco/chi2_NC_long_HadTop_el.pdf}}\\
\caption{}
...
\FloatBarrier
			...
			And also, it is better to use MVA with more variables than just $m_{jj}$ and $m_{jjb}$. Through there are 3 variables sets here to input and train. There are the variables of each combination in an event.
	...
	\label{ssec:bbsep}
		...
		The study is worked under the signal simulation sample($t$$\bar{t}$ Monte Carlo). All the selected physical objects include jets and leptons will be matched to particle with the generator-level information in MC sample with the same $\Delta R$ < 0.4 method.(Eq.\ref{eq:gen_matching})
		...
		With these 3 classification as a criteria of goodness, there are the result 
